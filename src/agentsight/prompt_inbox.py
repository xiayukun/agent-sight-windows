...
PROMPT_INBOX_SCHEMA = "agentsight_prompt_inbox_v1"
PROMPT_INBOX_ENTRY_SCHEMA = "agentsight_prompt_inbox_entry_v1"
PROMPT_INBOX_REPORT_SCHEMA = "agentsight_prompt_inbox_report_v1"
...
def prompt_inbox_status(*, path: Path | None = None, now_ms: int | None = None) -> dict[str, Any]:
    ...
    for entry in entries:
        ...
        if status == "claimed" and int(entry.get("claim_expires_at_ms") or 0) <= now:
            expired_claims += 1
    return {
        "object_type": "AgentSightPromptInboxStatus",
...
def append_prompt(
    ...
    *,
    ...
    with _prompt_inbox_guard(inbox_path):
        entries = _read_entries(inbox_path)
        entry = {
            "object_type": "AgentSightPromptInboxEntry",
...
def _prompt_inbox_report(
    *,
    ...
    return {
        "object_type": "AgentSightPromptInboxReport",
