...
from agentsight.tray.state import default_agent_dir
from agentsight.tray.viewers import build_timeline_model


class _InMemoryDecodedFrame:
    """Small in-memory image stand-in used when Qt bindings are unavailable."""
    ...
    def copy(self) -> "_InMemoryDecodedFrame":
        return _InMemoryDecodedFrame(data=bytes(self.data), width=self._width, height=self._height, mode=self.mode)
...
def launch_timeline_viewer_process(*, mode: str = "timeline") -> dict[str, Any]:
    ...
    if exe.exists():
        ...
        command = [sys.executable, "-B", "-m", "agentsight.tray.timeline_viewer", "--mode", mode]
        cwd = source_root
        env["PYTHONPATH"] = str(source_root / "src") + os.pathsep + env.get("PYTHONPATH", "")
        launch_kind = "source_qt_viewer"
    else:
        command = [sys.executable, "-m", "agentsight.tray.timeline_viewer", "--mode", mode]
        cwd = Path.cwd()
    ...
    process = subprocess.Popen(
        ...
        **_visible_viewer_subprocess_kwargs(),
    )
    report = {
...
def _source_repo_root_for_launcher() -> Path | None:
    ...
    for root in candidates:
        try:
            if (root / "src" / "AgentSight" / "tray" / "timeline_viewer.py").exists():
                return root
        except Exception:
            continue
    return None
...
def _visible_viewer_subprocess_kwargs() -> dict[str, Any]:
    """Launch the timeline viewer as a visible GUI window.

    The tray and helper commands run hidden, but the timeline viewer is the UI
    the user explicitly requested. Passing CREATE_NO_WINDOW or
    STARTF_USESHOWWINDOW/SW_HIDE here creates a live process with no visible
    top-level window.
    """
    if not is_windows():
        return {}
...
def decode_frame_to_qimage(frame: dict[str, Any]) -> tuple[Any | None, dict[str, Any]]:
    ...
    try:
        ...
        image, report = decode_segment_frame_to_image(restore_ref)
        hash_ok = bool(report.get("hash_ok", True))
        qimage = _pil_image_to_qimage_or_memory(image)
        return qimage.copy(), {
...
def _pil_image_to_qimage_or_memory(image: Any) -> Any:
    """Return a Qt QImage when PySide6 is present, otherwise keep pixels in memory."""
    rgba = image.convert("RGBA")
    data = rgba.tobytes("raw", "RGBA")
    try:
        from PySide6.QtGui import QImage
    except Exception:
        return _InMemoryDecodedFrame(data=data, width=rgba.width, height=rgba.height, mode="RGBA")
    format_enum = getattr(QImage, "Format", QImage)
    image_format = getattr(format_enum, "Format_RGBA8888", None)
    if image_format is None:
        image_format = getattr(QImage, "Format_RGBA8888")
    qimage = QImage(data, rgba.width, rgba.height, rgba.width * 4, image_format)
    return qimage.copy()


def _timeline_playback_can_update_selection(*, playback_timer_active: bool, syncing_to_player: bool) -> bool:
    """Return whether playback callbacks may move the selected timeline frame.

    A user click/drag/slider navigation first pauses the timeline timer. Any
    already queued timeout or media-position callback from the old playback
    generation must then become a no-op instead of briefly writing back an old
    frame before the clicked frame settles.
    """
    return bool(playback_timer_active) and not bool(syncing_to_player)


def _timeline_segment_end_can_advance(*, playback_timer_active: bool, pending_seek_ms: int | None) -> bool:
    """Return whether an EndOfMedia callback may advance to the next segment."""
    return bool(playback_timer_active) and pending_seek_ms is None


def _timeline_user_selection_should_emit(*, selected_index: int, nearest_index: int, force: bool = False) -> bool:
    """Return whether a strip user action should enter the user-navigation path.

    A plain click on the already-selected bar still has side effects: it must
    pause playback, clear pending async seeks, and re-prime the clicked frame.
    Drag hover updates can stay quieter to avoid repeated same-frame seeks.
    """
    return bool(force) or int(nearest_index) != int(selected_index)
    ...
    def __new__(cls, *args: Any, **kwargs: Any) -> Any:
        ...
        class _FrameStrip(QWidget):
            ...
            def _frame_x(self, frame: dict[str, Any]) -> float:
                try:
                    timestamp_ms = int(frame.get("timestamp_ms") or self.outer.timeline_start_ms)
                except Exception:
                    timestamp_ms = int(self.outer.timeline_start_ms)
                delta_ms = max(0, timestamp_ms - int(self.outer.timeline_start_ms))
                return (delta_ms / 1000.0) * self.px_per_second

            def _bar_x(self, frame: dict[str, Any]) -> float:
            ...
            def mousePressEvent(self, event: Any) -> None:
                ...
                self.outer._begin_user_navigation()

            def mouseMoveEvent(self, event: Any) -> None:
                if not hasattr(self, "_press_x"):
                    return
                x = float(event.position().x())
                if not self.dragging and abs(x - float(self._press_x)) < 4.0:
                    return
                self.dragging = True
                self._select_at(x, force=False)

            def mouseReleaseEvent(self, event: Any) -> None:
                self._select_at(float(event.position().x()), force=True)
                self.dragging = False
                if hasattr(self, "_press_x"):
                    delattr(self, "_press_x")

            def _select_at(self, x_pos: float, *, force: bool = False) -> None:
                if not self.outer.frames:
                    return
                content_x = x_pos + self.scroll_x
                nearest = min(
                    range(len(self.outer.frames)),
                    key=lambda idx: (abs(self._frame_x(self.outer.frames[idx]) - content_x), idx),
                )
                if _timeline_user_selection_should_emit(
                    selected_index=self.outer.selected_index,
        ...
        class _TimelineWindow(QMainWindow):
            def __init__(self, *, max_frames: int = 500, initial_mode: str = "timeline") -> None:
                ...
                self.duration_limit_ms: int | None = 3_600_000
                self.model = build_timeline_model(max_frames=max_frames, max_logs=500, duration_limit_ms=self.duration_limit_ms)
            ...
            def _build_ui(self, QSplitter: Any, QWidget: Any, QVBoxLayout: Any, QHBoxLayout: Any, QPushButton: Any, QLabel: Any, QComboBox: Any, QSlider: Any, QTreeWidget: Any, QTreeWidgetItem: Any, QScrollBar: Any) -> None:
                ...
                refresh_button.clicked.connect(self.refresh_model)
            ...
            def reload_model(self, *, keep_selected_time: bool = False) -> None:
                ...
                model = build_timeline_model(max_frames=self.max_frames, max_logs=500, duration_limit_ms=self.duration_limit_ms)
            ...
            def refresh_model(self) -> None:
                self.duration_limit_ms = 3_600_000
                self.max_frames = 500
                self.reload_model(keep_selected_time=True)

            def load_previous_frames(self) -> None:
                self._begin_user_navigation()
                self.duration_limit_ms = None
                self.max_frames += 500
                self.reload_model(keep_selected_time=True)
            ...
            def _on_player_position_changed(self, position_ms: int) -> None:
                if not _timeline_playback_can_update_selection(
                    playback_timer_active=self.timeline_play_timer.isActive(),
                    syncing_to_player=self._syncing_to_player,
                ):
                    return
            ...
            def _on_player_media_status_changed(self, status: Any) -> None:
                ...
                elif status == QMediaPlayer.MediaStatus.EndOfMedia and _timeline_segment_end_can_advance(
                    playback_timer_active=self.timeline_play_timer.isActive(),
                    pending_seek_ms=self.pending_seek_ms,
                ):
            ...
            def _play_next_segment_if_available(self) -> None:
                if not _timeline_segment_end_can_advance(
                    playback_timer_active=self.timeline_play_timer.isActive(),
                    pending_seek_ms=self.pending_seek_ms,
                ):
                    return
            ...
            def _begin_user_navigation(self) -> None:
