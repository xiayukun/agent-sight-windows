...
from agentsight.runtime_platform import is_windows, platform_system_label
from agentsight.tray.actions import emergency_stop
from agentsight.tray.state import boundary_facts


EMERGENCY_HOTKEY_SCHEMA = "agentsight_emergency_hotkey_v1"
WH_KEYBOARD_LL = 13
WM_KEYDOWN = 0x0100
WM_KEYUP = 0x0101
...
def default_hotkey_policy() -> dict[str, Any]:
    return {
        "object_type": "AgentSightEmergencyHotkeyPolicy",
...
def build_hotkey_description() -> dict[str, Any]:
    windows = _is_windows()
    return {
        "object_type": "AgentSightEmergencyHotkeyDescription",
        "schema": EMERGENCY_HOTKEY_SCHEMA,
        "platform": "Windows" if windows else platform_system_label(),
        "available": windows,
        "policy": default_hotkey_policy(),
        "run_command": "agentsight-tray emergency-hotkey run",
        "describe_command": "agentsight-tray emergency-hotkey describe",
        "host_input_sent": False,
        "host_sent_event_count": 0,
        "boundary": boundary_facts(),
    }
...
def build_hotkey_trigger_report(
    *,
    ...
    return {
        "object_type": "AgentSightEmergencyHotkeyTriggerReport",
        "schema": EMERGENCY_HOTKEY_SCHEMA,
...
class EmergencyHotkeyMonitor:
    ...
    def start(self) -> dict[str, Any]:
        if self._hook_handle:
            return {
                "object_type": "AgentSightEmergencyHotkeyStartReport",
                "schema": EMERGENCY_HOTKEY_SCHEMA,
    ...
    def _stopped_report(self, reason: str) -> dict[str, Any]:
        return {
            "object_type": "AgentSightEmergencyHotkeyRunReport",
            "schema": EMERGENCY_HOTKEY_SCHEMA,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="AgentSight physical emergency hotkey monitor.")
    ...
    try:
        ...
        report = {
            "object_type": "AgentSightEmergencyHotkeyRunFailure",
            "schema": EMERGENCY_HOTKEY_SCHEMA,
