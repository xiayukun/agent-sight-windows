...
from agentsight.tray.state import boundary_facts, default_agent_dir, default_evidence_root, default_tray_config_file


OPERATION_LOG_SCHEMA = "agentsight_operation_log_v1"
TIMELINE_VIEWER_SCHEMA = "agentsight_timeline_viewer_v1"
...
def append_operation_log(entry: dict[str, Any], *, path: Path | None = None) -> dict[str, Any]:
    ...
    payload = {
        "object_type": "AgentSightOperationLogEntry",
    ...
    return {
        "object_type": "AgentSightOperationLogAppendReport",
        "schema": OPERATION_LOG_SCHEMA,
...
def public_operation_log_entry(
    *,
    ...
    is_public_do = public_op == "do" or response_payload.get("schema") == "agentsight_do_v1" or response_payload.get("object_type") == "DoResult"
...
def build_timeline_model(
    *,
    max_frames: int = 500,
    max_logs: int = 200,
    right_wall_ms: int | None = None,
    duration_limit_ms: int | None = 3_600_000,
) -> dict[str, Any]:
    opened_at_ms = int(time.time() * 1000)
    effective_right_wall_ms = opened_at_ms if right_wall_ms is None else int(right_wall_ms)
    scanned_frames = _scan_segment_frames(default_evidence_root(), max_frames=None)
    frames, window_report = _select_timeline_frame_window(
        scanned_frames,
    ...
    return {
        "object_type": "AgentSightTimelineModel",
        ...
        "timeline_config": {
            ...
            "scanned_frame_count": window_report["scanned_frame_count"],
            "eligible_frame_count": window_report["eligible_frame_count"],
            "duration_filtered_frame_count": window_report["duration_filtered_frame_count"],
            "future_frame_count_excluded": window_report["future_frame_count_excluded"],
...
def _select_timeline_frame_window(
    frames: list[dict[str, Any]],
    *,
    max_frames: int,
    right_wall_ms: int,
    duration_limit_ms: int | None,
) -> tuple[list[dict[str, Any]], dict[str, int]]:
    frame_limit = max(1, int(max_frames))
    sorted_frames = sorted(frames, key=lambda frame: int(frame.get("timestamp_ms") or 0))
    eligible = [frame for frame in sorted_frames if int(frame.get("timestamp_ms") or 0) <= right_wall_ms]
    future_count = len(sorted_frames) - len(eligible)
    if duration_limit_ms is None:
        duration_filtered = eligible
    else:
        left_wall_ms = right_wall_ms - max(0, int(duration_limit_ms))
        duration_filtered = [frame for frame in eligible if int(frame.get("timestamp_ms") or 0) >= left_wall_ms]
    ...
    return selected, {
        ...
        "duration_filtered_frame_count": len(duration_filtered),
        "future_frame_count_excluded": future_count,
    }


def _scan_segment_frames(root: Path, *, max_frames: int | None) -> list[dict[str, Any]]:
    if not root.exists():
        return []
    frames: list[dict[str, Any]] = []
    try:
        from agentsight.segments.mkv_container import MKV_CONTAINER_MODEL, iter_mkv_frames
        ...
        frames.append(
            {
                ...
                "logical_frame_id": record.get("logical_frame_id", frame_id),
                "logical_frame_index": record.get("logical_frame_index", record.get("frame_index")),
                "physical_frame_id": record.get("physical_frame_id", frame_id),
    ...
    frames.extend(_scan_binary_agseg_segment_frames(root, max_frames=max_frames))
...
def _scan_binary_agseg_segment_frames(root: Path, *, max_frames: int | None) -> list[dict[str, Any]]:
    try:
        ...
        return []
    frames: list[dict[str, Any]] = []
    for segment_path in sorted(root.rglob("*.agseg")):
        try:
            reader = BinarySegmentReader(segment_path)
        except Exception:
            continue
        for record in reader.manifest.get("frames") or []:
            frame_id = str(record.get("frame_id") or "")
            if not frame_id:
                continue
            timestamp_ms = int(_timestamp_ms_from_frame_record(record, fallback=segment_path.stat().st_mtime))
            frames.append(
                {
                    "index": 0,
                    "path": None,
                    "uri": None,
                    "name": f"{reader.manifest.get('segment_id')}-{frame_id}",
                    "timestamp_ms": timestamp_ms,
                    "size_bytes": int(segment_path.stat().st_size) if segment_path.exists() else 0,
                    "raw_or_derived": "raw_segment_index_metadata",
                    "preview_policy": "on_demand_only",
                    "canonical_source": BINARY_CONTAINER_MODEL,
                    "storage_format": "binary_agseg",
                    "segment_id": reader.manifest.get("segment_id"),
                    "segment_frame_id": frame_id,
                    "segment_frame_kind": record.get("frame_kind"),
                    "segment_source": record.get("source"),
                    "segment_path_abs": str(segment_path.resolve()),
...
def _timestamp_ms_from_frame_record(record: dict[str, Any], *, fallback: float) -> int:
    if record.get("timestamp_ms") is not None:
        try:
            return int(record.get("timestamp_ms"))
        except (TypeError, ValueError):
            pass
...
def _iter_view_records(value: Any) -> list[dict[str, Any]]:
    ...
    if isinstance(value, dict):
        ...
        if value.get("schema") == "agentsight_view_index_v1" and isinstance(value.get("public_record"), dict):
...
def materialize_look_preview_cache(preview_ref: dict[str, Any], *, output_dir: Path | None = None) -> dict[str, Any]:
    ...
    try:
        from agentsight.segments.decoder import decode_segment_region_to_png
