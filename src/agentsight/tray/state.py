...
from agentsight.caller_lock import caller_lock_status, default_caller_lock_file
from agentsight.operator_notifications import default_operator_notification_file, notification_status
from agentsight.operator_workflow import workflow_status
from agentsight.prompt_inbox import default_prompt_inbox_file, prompt_inbox_status
from agentsight.runtime_platform import is_windows


TRAY_STATUS_SCHEMA = "agentsight_tray_status_v1"
TRAY_CONFIG_SCHEMA = "agentsight_tray_config_v1"
TRAY_POLICY_SCHEMA = "agentsight_tray_policy_v1"
OPERATOR_CONTROL_POLICY_SCHEMA = "agentsight_operator_control_policy_v1"
EMERGENCY_STOP_SCHEMA = "agentsight_emergency_stop_v1"
...
def default_recording_policy() -> dict[str, Any]:
    return {
        "object_type": "AgentSightTrayConfig",
...
def normalize_recording_policy(payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if isinstance(idle, dict):
        ...
        idle["fps"] = _coerce_float(idle.get("fps"), default=0.1, minimum=0.1, maximum=60.0)
    recording = policy.get("recording")
    if isinstance(recording, dict):
        recording.pop("segment", None)
        action = recording.get("action_capture")
        if isinstance(action, dict):
            action.pop("max_post_action_frames", None)
...
def _coerce_gb_to_mb(value: Any, *, default_mb: int, minimum_mb: int, maximum_mb: int) -> int:
    try:
        parsed_gb = float(value)
    except (TypeError, ValueError):
        return int(default_mb)
    parsed_mb = int(round(parsed_gb * 1024))
    return max(int(minimum_mb), min(int(maximum_mb), parsed_mb))
...
def apply_recording_policy_settings(settings: dict[str, Any], *, path: Path | None = None) -> dict[str, Any]:
    ...
    if "idle_fps" in settings:
        idle["fps"] = _coerce_float(settings["idle_fps"], default=0.1, minimum=0.1, maximum=60.0)
    ...
    if "post_action_fps" in settings:
        action["post_action_fps"] = _coerce_int(settings["post_action_fps"], default=1, minimum=1, maximum=60)
    ...
    if "post_action_duration_seconds" in settings:
        duration_seconds = _coerce_float(settings["post_action_duration_seconds"], default=10.0, minimum=0.001, maximum=60.0)
        action["post_action_duration_ms"] = int(round(duration_seconds * 1000))
    action.pop("max_post_action_frames", None)
    ...
    if "max_storage_gb" in settings:
        payload["max_storage_mb"] = _coerce_gb_to_mb(settings["max_storage_gb"], default_mb=5120, minimum_mb=256, maximum_mb=1048576)
    if "min_free_disk_mb" in settings:
        payload["min_free_disk_mb"] = _coerce_int(settings["min_free_disk_mb"], default=5120, minimum=256, maximum=1048576)
    if "min_free_disk_gb" in settings:
        payload["min_free_disk_mb"] = _coerce_gb_to_mb(settings["min_free_disk_gb"], default_mb=5120, minimum_mb=256, maximum_mb=1048576)
    if "segment_bucket_granularity" in settings:
        payload["segment_bucket_granularity"] = _coerce_segment_bucket_granularity(settings["segment_bucket_granularity"])
    if "segment_storage_format" in settings:
        payload["segment_storage_format"] = _coerce_segment_storage_format(settings["segment_storage_format"])
    if "segment_image_encoding" in settings:
        payload["segment_image_encoding"] = _coerce_image_encoding(settings["segment_image_encoding"])
    if "segment_image_quality" in settings:
        payload["segment_image_quality"] = _coerce_int(settings["segment_image_quality"], default=70, minimum=1, maximum=100)
    if "segment_image_lossless" in settings:
        payload["segment_image_lossless"] = _coerce_bool(settings["segment_image_lossless"], False)
    ...
    return {
        "object_type": "AgentSightTrayConfigUpdateReport",
        "schema": TRAY_CONFIG_SCHEMA,
        "updated": True,
...
def build_operator_control_policy(
    *,
    ...
    return {
        "object_type": "AgentSightOperatorControlPolicy",
        ...
        "reason": reason or ("operator_allowed_agentsight" if enabled else "operator_paused_agentsight"),
...
def load_operator_control_policy(path: Path | None = None) -> dict[str, Any]:
    ...
    normalized = build_operator_control_policy(
        real_control_enabled=enabled,
        reason=str(payload.get("reason") or ("operator_allowed_agentsight" if enabled else "operator_paused_agentsight")),
...
def write_operator_control_policy(
    *,
    ...
    return {
        "object_type": "AgentSightOperatorControlPolicyWriteReport",
        "schema": "agentsight_operator_control_policy_write_report_v1",
...
def _service_state_from_session_supervisor_state(payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    if isinstance(health, dict):
        ...
        if service_status:
            return {
                "object_type": "AgentSightServiceStateProjection",
                "schema": "agentsight_service_state_projection_v1",
...
def build_emergency_stop_payload(reason: str, *, now_ms: int | None = None) -> dict[str, Any]:
    return {
        "object_type": "AgentSightEmergencyStop",
...
def write_emergency_stop(
    *,
    ...
    watchdog_path.write_text("AgentSight emergency stop active. Clear explicitly to allow restart.\n", encoding="utf-8")
    return {
        "object_type": "AgentSightEmergencyStopWriteReport",
        "schema": "agentsight_emergency_stop_write_report_v1",
...
def clear_emergency_stop(
    *,
    ...
    return {
        "object_type": "AgentSightEmergencyStopClearReport",
        "schema": "agentsight_emergency_stop_clear_report_v1",
        "emergency_stop_file": str(emergency_path),
        "watchdog_stop_file": str(watchdog_path),
...
def load_tray_status(
    *,
    ...
    return {
        "object_type": "AgentSightTrayStatus",
        ...
        "controls": {
            ...
            "physical_emergency_hotkey": {
                ...
                "entrypoint": "agentsight-tray emergency-hotkey run",
                "describe_entrypoint": "agentsight-tray emergency-hotkey describe",
            ...
            "tray_icon_gui_entrypoint": "agentsight-tray-gui",
        },
        "host_input_sent": False,
        "host_sent_event_count": 0,
        "boundary": boundary_facts(),
