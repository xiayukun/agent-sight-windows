...
from agentsight.host_agent.service_state import ServiceStateError, validate_service_state


def main() -> int:
    parser = argparse.ArgumentParser(description="Run AI-facing P0 scenarios through a discovered AgentSight Host Agent.")
    ...
    *,
    ...
    if not wait["found"]:
        ...
        return {
            ...
            "service_state_read": _redact_sensitive(service_state_read),
            "readiness": _redact_sensitive(readiness),
            "wait": wait,
            "installer_report": _redact_sensitive(_read_installer_report()),
            "agent_report": _redact_sensitive(_read_agent_report()),
...
def _build_ai_readiness_report(
    *,
    ...
    service_state_source = "service_state_file" if service_state_valid else None
    embedded_service_state_error = None
    if not service_state_valid and isinstance(health_data, dict) and isinstance(health_data.get("service_state"), dict):
        try:
            service_state = validate_service_state(health_data["service_state"])
            service_state_valid = True
            service_state_source = "health_embedded_service_state"
        except ServiceStateError as exc:
            embedded_service_state_error = str(exc)
    ...
    can_attempt = all(
        ...
        for key in (
            "service_state_valid",
            "service_state_allows_control",
            "discovery_found",
            "discovery_has_token",
            "discovery_has_health_url",
            "health_request_ok",
            "health_allows_control",
        )
    )
    ...
    return {
        "object_type": "AgentSightHostAgentReadinessReport",
        "schema": "agentsight_host_agent_readiness_v1",
        ...
        "wait": wait,
        "service_state_read": service_state_read,
        "service_state_source": service_state_source,
        "health_embedded_service_state_error": embedded_service_state_error,
        "service_status": service_status,
...
def _readiness_blockers(
    *,
    ...
    blockers: list[str] = []
    if not readiness_checks["service_state_valid"] and not readiness_checks["service_state_read"]:
...
def _readiness_suggestions_from_blockers(blockers: list[str]) -> list[str]:
    ...
    if (
        ...
        suggestions.append("install_or_start_agentsight_host_agent_in_visible_interactive_session")
...
def _read_installer_report() -> dict[str, Any] | None:
    ...
    try:
        ...
        return {
            "object_type": "AgentSightHostAgentInstallerReportReadFailure",
...
def _read_agent_report() -> dict[str, Any] | None:
    ...
    try:
        ...
        return {
            "object_type": "AgentSightHostAgentRuntimeReportReadFailure",
...
def _scenario_report(
    ...
    *,
    ...
    return {
        ...
        "health": _redact_sensitive(health),
        "readiness": _redact_sensitive(readiness),
        "service_state_file": readiness_report.get("service_state_file"),
        "service_state_read": _redact_sensitive(readiness_report.get("service_state_read")),
...
def _redact_sensitive(value: Any) -> Any:
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            if str(key).lower() in {"token", "auth_header", "authorization"}:
                safe[key] = _redacted_sensitive_string(str(item)) if item is not None else "<redacted>"
            else:
    ...
    if isinstance(value, str):
        return _redact_bearer_string(value)
    return value


def _redacted_sensitive_string(value: str) -> str:
    redacted = _redact_bearer_string(value)
    if redacted != value:
        return redacted
    return "<redacted>"


def _redact_bearer_string(value: str) -> str:
    redacted = re.sub(r"(?i)Bearer\s+[^\s\"']+", "Bearer ***", value)
    return redacted
...
def _read_service_state_report(path: Path) -> dict[str, Any]:
    if not path.exists():
        return {
            "object_type": "AgentSightServiceStateReadReport",
            "path": str(path),
            "read_ok": False,
            "valid": False,
            "status": "missing",
    ...
    return {
        ...
        "validator": "agentsight.host_agent.service_state.validate_service_state",
...
def _default_installer_report_file() -> Path:
    base = os.environ.get("LOCALAPPDATA")
    if base:
        return Path(base) / "AgentSight" / "last-installer-report.json"
    return Path.home() / ".agentsight" / "last-installer-report.json"
