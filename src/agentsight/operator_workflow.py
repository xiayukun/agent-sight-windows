...
from agentsight.prompt_inbox import (
    claim_next_prompt,
    complete_prompt,
    prompt_inbox_status,
)
...
OPERATOR_WORKFLOW_SCHEMA = "agentsight_operator_workflow_v1"
OPERATOR_WORKFLOW_REPORT_SCHEMA = "agentsight_operator_workflow_report_v1"
...
def workflow_status(
    *,
    ...
    return {
        "object_type": "AgentSightOperatorWorkflowStatus",
...
def _recommended_next(
    *,
    ...
    if pending_prompts and caller_id:
        recommendations.append("agentsight-tray ai-workflow claim --caller-id <stable-id>")
    elif pending_prompts:
        recommendations.append("choose_stable_caller_id_then_claim_prompt")
    if pending_notifications:
        recommendations.append("agentsight-tray notify claim --caller-id <stable-id>")
...
def _claimed_notification_text(*, caller_id: str, prompt: dict[str, Any]) -> str:
    ...
    return (
        f"AgentSight prompt claimed: prompt_id={prompt.get('prompt_id')}; "
...
def _completed_notification_text(*, caller_id: str, prompt_id: str) -> str:
    return (
        f"AgentSight prompt marked complete by caller bookkeeping: prompt_id={prompt_id}; "
...
def _stage_completion_summary(
    *,
    ...
    parts = [
        f"AgentSight stage update: {normalized_stage} {normalized_result}.",
    ]
    if summary_text:
...
def _workflow_report(
    *,
    ...
    return {
        "object_type": "AgentSightOperatorWorkflowReport",
