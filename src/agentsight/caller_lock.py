...
CALLER_LOCK_SCHEMA = "agentsight_single_caller_lock_v1"
CALLER_LOCK_REPORT_SCHEMA = "agentsight_single_caller_lock_report_v1"
...
def caller_lock_status(*, path: Path | None = None, now_ms: int | None = None) -> dict[str, Any]:
    ...
    return {
        "object_type": "AgentSightSingleCallerLockStatus",
    ...
    *,
    ...
    if not normalized:
        return False, 400, _caller_lock_report(
            ...
            detail="real-control endpoints require caller_id in the JSON body or X-AgentSight-Caller header",
    ...
    with _caller_lock_guard(lock_path):
        ...
        payload = {
            "object_type": "AgentSightSingleCallerLock",
            "schema": CALLER_LOCK_SCHEMA,
            "active": True,
...
def check_single_caller_lock(
    ...
    *,
    ...
    """Check caller-lock compatibility without acquiring or refreshing it."""
    lock_path = path or default_caller_lock_file()
    now = int(now_ms if now_ms is not None else time.time() * 1000)
    normalized = normalize_caller_id(caller_id)
    if not normalized:
        ...
        return True, 200, _caller_lock_report(
            status="caller_lock_not_checked_no_caller",
            failure_code=None,
            detail="no caller identity was provided for this read-only preflight; caller lock was not acquired or refreshed",
            request_path=request_path,
            lock_file=lock_path,
            now_ms=now,
        )
    ...
    with _caller_lock_guard(lock_path):
        ...
        if isinstance(lock, dict) and lock.get("active"):
            ...
            if existing_hash == owner_hash and expires_at_ms > now:
                return True, 200, _caller_lock_report(
                    status="caller_lock_owned_by_request_caller",
                    failure_code=None,
                    detail="caller already owns the real-control lock; read-only preflight did not refresh it",
                    request_path=request_path,
                    lock_file=lock_path,
                    now_ms=now,
                    owner_hash_12=owner_hash[:12],
            ...
            if expires_at_ms <= now:
                return True, 200, _caller_lock_report(
                    status="caller_lock_stale_available",
                    failure_code=None,
                    detail="existing caller lock is stale; read-only preflight did not acquire a new lock",
                    request_path=request_path,
                    lock_file=lock_path,
                    now_ms=now,
                    owner_hash_12=existing_hash[:12] if existing_hash else None,
                    owner_hint=lock.get("owner_hint"),
                    expires_at_ms=expires_at_ms,
                    requested_caller_hash_12=owner_hash[:12],
                    requested_caller_hint=caller_hint(normalized),
    ...
    return True, 200, _caller_lock_report(
        status="caller_lock_available",
        failure_code=None,
        detail="no active conflicting caller lock was found; read-only preflight did not acquire a lock",
        request_path=request_path,
        lock_file=lock_path,
        now_ms=now,
        requested_caller_hash_12=owner_hash[:12],
...
def _caller_lock_report(
    *,
    ...
    return {
        "object_type": "AgentSightSingleCallerLockReport",
