...
WINDOW_SEMANTICS_PATTERN = re.compile(
    ...
    r"AgentSightTrayWindow|tray_window_class|\bhwnd\b|ctypes\.windll\.user32|"
...
def verify_p3e(inputs: VerifierInputs) -> dict[str, Any]:
    ...
    return {
        ...
        "schema": "agentsight_p3e_evidence_verifier_report_v1",
...
def load_default_actual_token(discovery_path: Path | None = None) -> str | None:
    path = discovery_path or Path.home() / "AppData" / "Local" / "AgentSight" / "host-agent.json"
    if not path.exists():
        return None
    try:
...
def _is_do_result(obj: dict[str, Any]) -> bool:
    return obj.get("schema") in {"agentsight_do_result_v1", "agentsight_do_v1"} or obj.get("object_type") in {
        "DoResult",
        "P3ADoResult",
    }
