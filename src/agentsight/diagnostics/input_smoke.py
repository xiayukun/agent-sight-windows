...
from agentsight.channels.windows_software import WindowsSoftwareInputChannel
from agentsight.channels.windows_software.sendinput_backend import Win32InputBackend
...
def apply_manual_report_profile(report: dict[str, Any], profile: str = "full") -> dict[str, Any]:
    ...
    full_report["how_to_expand"] = (
        "already_full"
        if requested == "full"
        else "rerun with AGENTSIGHT_REPORT_PROFILE=full for the full manual smoke report"
    )
...
def _compact_report_from_receipt(report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "how_to_expand": "rerun with AGENTSIGHT_REPORT_PROFILE=full for the full manual smoke report",
    }
