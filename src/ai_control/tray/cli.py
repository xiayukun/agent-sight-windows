...
from ai_control.operator_workflow import (
    workflow_claim_next,
...
from ai_control.prompt_inbox import append_prompt, claim_next_prompt, complete_prompt, list_prompts, prompt_inbox_status
from ai_control.tray.ai_status import ai_status_report
from ai_control.tray.actions import allow_ai_control, clear_emergency, emergency_stop, pause_ai_control
from ai_control.tray.gui import build_tray_gui_description
from ai_control.tray.hotkey import build_hotkey_description, main as hotkey_main
from ai_control.tray.state import boundary_facts, load_tray_status
from ai_control.tray.viewers import materialize_look_preview_cache_from_operation_log
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="AI-Control tray-visible control surface.")
    ...
    pause.add_argument("--reason", default="operator_paused_ai_control")
    allow = operator_commands.add_parser("allow", description="Allow AI real control after an operator pause.")
    allow.add_argument("--reason", default="operator_allowed_ai_control")
...
def _unsupported_look_preview_command_report(command: str) -> dict[str, Any]:
    return {
        "object_type": "AIControlLookPreviewCliReport",
...
def operator_control_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if command == "pause":
        return pause_ai_control(args.reason)
    if command == "allow":
        return allow_ai_control(args.reason)
...
def ai_help_report() -> dict[str, Any]:
    return {
        "object_type": "AIControlTrayAIHelp",
        "schema": "ai_control_tray_ai_help_v1",
        "recommended_flow": [
            "Run ai-control-tray status before using Host Agent endpoints.",
            "For a compact ordinary-AI preflight, run ai-control-tray ai-status --caller-id <stable-id> first.",
            "Choose a stable caller_id and send it on real-control requests as X-AI-Control-Caller or JSON caller_id.",
            ...
            "For mobile or relay-originated work, read ai-control-tray prompt-inbox status, then claim work with ai-control-tray prompt-inbox claim --caller-id <stable-id>.",
            "A claimed prompt is an operator instruction queue item; the tool does not judge business success when it is completed.",
            "Queue progress updates with ai-control-tray notify enqueue, then mark them sent or blocked after a visible delivery attempt.",
            "Use ai-control-tray notify claim --caller-id <stable-id> to claim one pending progress notification for visible GUI delivery.",
            "Use ai-control-tray notify prepare-delivery --caller-id <stable-id> after claim to get a visible-GUI-only draft and checklist.",
            ...
            "Use ai-control-tray ai-workflow status to see prompt and notification next steps, then claim or report through that workflow bridge.",
            "P1-V: use ai-control-tray ai-workflow stage-report to queue a standardized stage progress notification for later visible delivery; it does not send messages and does not judge business success.",
            "For human review only, use ai-control-tray look-preview materialize --log-index N --preview-index M to regenerate one derived review cache image from a logged /look view; this is not an ordinary AI public look/do step.",
            "Use ai-control-tray ai-status to summarize readiness, operator-control, caller-lock, workflow counts, and public screen/look/do flow hints without screen capture or input.",
            "Use ai-control-tray operator-control status before real-control work if you need the explicit operator permission state.",
            ...
            "Use ai-control-tray emergency-stop only when the operator asks for an immediate visible denial of AI control.",
            "Use ai-control-tray emergency-hotkey describe to inspect the physical Ctrl+Alt+Shift+Esc emergency stop monitor. The monitor ignores injected keyboard events.",
        ...
        "commands": {
            ...
            "gui-describe": "Print the Tray GUI capability model from the console CLI, useful when AIControlTrayGui.exe is packaged as a windowed app without stdout.",
