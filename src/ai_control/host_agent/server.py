...
from ai_control.caller_lock import (
...
from ai_control.channels.pointer_events import is_mouse_action, mouse_action_points, mouse_action_summary
...
from ai_control.channels.windows_software.observation import WindowsSoftwareObservationChannel
from ai_control.diagnostics.p0_real_input_closed_loop import (
    ARMING_REF,
    CONSENT_REF,
    P0_ARMING_FLAG,
    WindowsHostProbe,
...
from ai_control.host_agent.service_state import (
    build_service_health,
    project_host_agent_health_to_service_state,
    project_supervisor_child_health_to_service_state,
)
from ai_control.protocol.schemas import SchemaError, validate_post_observe, validate_request
...
from ai_control.tray.viewers import append_operation_log, public_operation_log_entry
from ai_control.visual_memory.post_observe import build_post_action_observation_window, should_stop_post_observe_sampling
...
def _build_host_agent_health_payload(
    *,
    ...
    return {
        "object_type": "AIControlHostAgentHealth",
        "schema": "ai_control_host_agent_health_v1",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the AI Control local host agent.")
    ...
    _write_last_agent_report(
        {
            "object_type": "AIControlHostAgentRuntimeReport",
            "schema": "ai_control_host_agent_runtime_report_v1",
    ...
    *,
    ...
    while True:
        ...
        if probe.get("watchdog_probe_status") == "unified_session_supervisor_enabled":
            report = {
                "object_type": "AIControlHostAgentWatchdogReport",
                "schema": "ai_control_host_agent_watchdog_v1",
...
def _watchdog_child_agent_command(
    *,
    ...
    if getattr(sys, "frozen", False):
        ...
        command = [sys.executable, "-m", "ai_control.host_agent.server"]
    command.extend(
        [
            "--host",
            host,
...
def default_agent_error_file() -> Path:
    base = os.environ.get("LOCALAPPDATA")
    if base:
        return Path(base) / "ai-control" / "last-agent-error.json"
    return Path.home() / ".ai-control" / "last-agent-error.json"


def default_watchdog_report_file() -> Path:
    base = os.environ.get("LOCALAPPDATA")
    if base:
        return Path(base) / "ai-control" / "last-watchdog-report.json"
    return Path.home() / ".ai-control" / "last-watchdog-report.json"
...
def default_watchdog_stop_file() -> Path:
    base = os.environ.get("LOCALAPPDATA")
    if base:
        return Path(base) / "ai-control" / "host-agent-watchdog.stop"
    return Path.home() / ".ai-control" / "host-agent-watchdog.stop"


def _unified_session_supervisor_enabled_file() -> Path:
    base = os.environ.get("LOCALAPPDATA")
    if base:
        return Path(base) / "ai-control" / "unified-session-supervisor.enabled"
    return Path.home() / ".ai-control" / "unified-session-supervisor.enabled"
...
def write_discovery_file(
    ...
    *,
    ...
    payload = {
        ...
        "schema": "discovery_v2",
        "legacy_schema": "ai_control_host_agent_discovery_v1",
        "url": url,
        "api": {
            "screen": "/screen",
            "look": "/look",
...
def _host_agent_visual_observe(
    *,
    ...
    report = {
        ...
        "schema": "ai_control_p0b_visual_observe_v1",
        "visual_session_id": visual_session_id,
...
def _host_agent_screen_layout(*, health: dict[str, Any] | None = None, armed: bool = True) -> dict[str, Any]:
...
def _host_historical_view_index_entry(
    *,
    ...
    return {
        "object_type": "AIControlViewIndexEntry",
        "schema": "ai_control_view_index_v1",
    ...
    *,
    ...
    for index, step in enumerate(seq, start=1):
        ...
        capture_windows.append(
            {
                "kind": "burst",
                "from": _format_hms_ms(input_started),
                "to": _format_hms_ms(input_started + 3.0),
                "reason": f"step_{index}_input",
            }
        )
...
def _host_agent_apply_recording_policy_defaults(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if not bool(action_capture.get("capture_post_action_frames")):
        return request
    fps = _coerce_int_range(action_capture.get("post_action_fps"), default=10, minimum=1, maximum=60)
    duration_ms = _coerce_int_range(action_capture.get("post_action_duration_ms"), default=10000, minimum=1, maximum=60000)
    max_frames = _coerce_int_range(action_capture.get("max_post_action_frames"), default=100, minimum=1, maximum=1000)
    frame_count = max(1, min(max_frames, int((fps * duration_ms + 999) // 1000)))
...
def _host_agent_fast_do_input(
    *,
    ...
    report = {
        ...
        "schema": "ai_control_pobserve_fast_do_input_v1",
        "visual_session_id": visual_session_id,
        "source_observation_ref": source_observation_ref,
        "route": route,
...
def _host_agent_visual_click(
    *,
    ...
    report = {
        ...
        "schema": "ai_control_p0b_visual_click_v1",
        "visual_session_id": visual_session_id,
...
def _host_agent_visual_input(
    *,
    ...
    report = {
        ...
        "schema": "ai_control_p0c_visual_input_v1",
        "visual_session_id": visual_session_id,
...
def _host_agent_visual_mouse(
    *,
    ...
    report = {
        ...
        "schema": "ai_control_p1a_mouse_action_v1",
        "visual_session_id": visual_session_id,
...
def _host_coordinate_integrity(
    ...
    *,
    ...
    screen_region = source_frame.get("screen_region")
    return {
        "schema": "ai_control_p1c_coordinate_integrity_v1",
...
def _pverify_preinput_check(
    ...
    *,
    ...
    return {
        "object_type": "AIControlPVerifyPreInputCheck",
        "schema": "ai_control_pverify_step_verification_v1",
...
def _review_media_for_frame(frame: dict[str, Any], *, request: dict[str, Any], role: str) -> dict[str, Any]:
    ...
    base: dict[str, Any] = {
        ...
        "schema": "ai_control_host_review_media_v1",
        "status": "not_generated",
...
def _host_agent_public_readiness_message(code: str) -> str:
    return {
        ...
        "SECURE_DESKTOP_ACTIVE": "A secure desktop is active; AI-Control reports the blocker and does not bypass it.",
        "UAC_SECURE_DESKTOP_ACTIVE": "A UAC secure desktop is active; AI-Control reports the blocker and does not control it.",
...
def _start_host_agent_idle_capture_loop(
    *,
    ...
    thread = threading.Thread(target=run, name="AIControlIdleCapture", daemon=True)
    thread.start()
    return thread
...
def _host_agent_public_blocked_response(
    *,
    ...
    report = {
        "object_type": "AIControlPublicReadinessBlocked",
        "schema": "ai_control_public_readiness_blocked_v1",
        "ok": False,
        "status": "blocked",
        "operation": operation,
...
def _host_agent_visual_failure(
    ...
    **extra: Any,
    ...
    return {
        ...
        "schema": "ai_control_p0b_visual_control_failure_v1",
        "status": status,
...
def _handler_class(*, runs_dir: str, arm_real_input: bool, token: str) -> type[BaseHTTPRequestHandler]:
    ...
    class HostAgentHandler(BaseHTTPRequestHandler):
        server_version = "AIControlHostAgent/0.1"
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if self.path == "/screen":
                ...
                if not readiness["ok"]:
                    ...
                    return
                report = _host_agent_screen_layout(health=health, armed=arm_real_input)
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            if self.path in {"/observe", "/click", "/mouse", "/input", "/look", "/do"}:
                ...
                try:
                    with visual_lock:
                        if self.path == "/look":
                            status, report = _host_agent_protocol_look(
                                visual_sessions=visual_sessions,
                                protocol_views=protocol_views,
                                runs_dir=runs_dir,
                                request=request,
                            ...
                            if tray_policy_post_observe_applied and isinstance(report, dict):
                                report["recording_policy"] = {
                                    ...
                                    "policy_basis": "recording.action_capture.post_action_fps/post_action_duration_ms/max_post_action_frames",
                                    "host_input_sent": False,
                                    "host_sent_event_count": 0,
                                    "boundary": _host_agent_boundary_facts(),
                                }
                        elif self.path == "/observe":
                            status, report = _host_agent_visual_observe(
                                visual_sessions=visual_sessions,
                                runs_dir=runs_dir,
                                request=request,
                            )
                        elif self.path == "/click":
                            status, report = _host_agent_visual_click(
                                visual_sessions=visual_sessions,
                                request=request,
                            )
                        elif self.path == "/mouse":
                            status, report = _host_agent_visual_mouse(
                                visual_sessions=visual_sessions,
                                request=request,
                            )
                        else:
                            status, report = _host_agent_visual_input(
                                visual_sessions=visual_sessions,
                                request=request,
                            )
                    ...
                    error_report = {
                        "object_type": "AIControlHostAgentErrorReport",
                        "schema": "ai_control_host_agent_error_v1",
        ...
        def _caller_lock_gate(self, request: dict[str, Any]) -> tuple[bool, int, dict[str, Any]]:
            return enforce_single_caller_lock(
                caller_id_from_request(request, header_value=self.headers.get("x-ai-control-caller")),
                request_path=self.path,
            )
            ...
            try:
                append_operation_log(
                    public_operation_log_entry(
                        ...
                        caller_hint=self.headers.get("x-ai-control-caller"),
                    )
                )
            except Exception:
                return
...
def _active_interactive_session(session: dict[str, Any]) -> bool:
    return bool(session.get("process_is_active_console_session") or session.get("process_session_is_wts_active"))
