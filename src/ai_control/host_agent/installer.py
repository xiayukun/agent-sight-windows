...
ONLOGON_TASK_NAME = "AIControlHostAgentOnLogon"


def main() -> int:
    parser = argparse.ArgumentParser(description="Install, start, check, or remove the AI Control Host Agent.")
...
def install_host_agent(
    *,
    ...
    report: dict[str, Any] = {
        "object_type": "AIControlHostAgentInstallerReport",
        "schema": "ai_control_host_agent_installer_v1",
...
def stop_running_host_agent(*, wait_seconds: float = 5.0) -> dict[str, Any]:
    ...
    if not discovery:
        return {
            "object_type": "AIControlHostAgentStopReport",
...
def _request_watchdog_stop() -> dict[str, Any]:
    ...
    try:
        ...
        path.write_text(
            json.dumps(
                {
                    "object_type": "AIControlHostAgentWatchdogStopRequest",
...
def _paths() -> dict[str, Path]:
    ...
    if local_appdata:
        agent_dir = Path(local_appdata) / "ai-control"
    else:
        agent_dir = Path.home() / ".ai-control"
    startup = Path(os.environ.get("APPDATA", str(Path.home()))) / "Microsoft" / "Windows" / "Start Menu" / "Programs" / "Startup"
    return {
        "agent_dir": agent_dir,
        "startup": startup,
        "cmd": agent_dir / "AIControlHostAgent.cmd",
        "vbs": startup / "AIControlHostAgent.vbs",
    }
    ...
    *,
    ...
    return "\r\n".join(
        [
            ...
            "set PYTHONPATH=src",
            (
                f'"{python_command}" -m ai_control.host_agent.server --watchdog '
...
def _start_via_interactive_task(task_path: Path, *, startup_vbs_path: Path) -> dict[str, Any]:
    ...
    task_name = "AIControlHostAgentInteractiveLaunch"
...
def _default_adjacent_agent_exe() -> Path | None:
    candidate = Path(sys.executable).with_name("AIControlHostAgent.exe")
