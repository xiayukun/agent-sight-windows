...
SERVICE_STATE_SCHEMA: Final = "ai_control_service_state_v1"
SERVICE_HEALTH_SCHEMA: Final = "ai_control_service_health_v1"
...
def service_state_schema() -> dict[str, Any]:
    return {
        "object_type": "AIControlServiceStateSchema",
        "schema": "ai_control_service_state_schema_v1",
...
def service_health_schema() -> dict[str, Any]:
    return {
        "object_type": "AIControlServiceHealthSchema",
        "schema": "ai_control_service_health_schema_v1",
...
def build_service_state(
    ...
    *,
    ...
    return {
        "object_type": "AIControlServiceState",
...
def build_service_health(state_or_status: dict[str, Any] | str) -> dict[str, Any]:
    ...
    can_attempt = status == OK_ACTIVE_DEFAULT_DESKTOP
    return {
        "object_type": "AIControlServiceHealth",
...
def classify_supervisor_child_health(
    *,
    ...
    return {
        ...
        "schema": "ai_control_supervisor_child_health_classification_v1",
        "service_status": status,
...
def classify_host_agent_health(
    *,
    ...
    if "kill_switch_active" in raw_blocker_set:
        ...
        signals.append("process_session_is_active_non_console_session")
        if not raw_can_attempt_real_control:
            status = "child_unhealthy"
    ...
    return {
        ...
        "schema": "ai_control_host_agent_service_state_classification_v1",
        "service_status": status,
        "can_attempt_real_control": status == OK_ACTIVE_DEFAULT_DESKTOP,
        "signals": signals,
        "raw_can_attempt_real_control": bool(raw_can_attempt_real_control),
...
def validate_service_state(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("object_type") != "AIControlServiceState":
        raise ServiceStateError("service state object_type must be AIControlServiceState")
...
def validate_service_health(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("object_type") != "AIControlServiceHealth":
        raise ServiceStateError("service health object_type must be AIControlServiceHealth")
