...
OPERATOR_NOTIFICATION_SCHEMA = "ai_control_operator_notification_v1"
OPERATOR_NOTIFICATION_REPORT_SCHEMA = "ai_control_operator_notification_report_v1"
...
def notification_status(*, path: Path | None = None, now_ms: int | None = None) -> dict[str, Any]:
    ...
    return {
        "object_type": "AIControlOperatorNotificationStatus",
...
def _notification_delivery_attention(
    *,
    ...
    if int(counts.get("claimed") or 0) > 0:
        ...
        next_cli = "ai-control-tray notify prepare-delivery --caller-id <stable-id>"
    elif int(counts.get("pending") or 0) > 0:
        next_action = "claim_pending_notification"
        next_cli = "ai-control-tray notify claim --caller-id <stable-id>"
...
def enqueue_notification(
    ...
    *,
    ...
    entry = {
        "object_type": "AIControlOperatorNotification",
        "schema": OPERATOR_NOTIFICATION_SCHEMA,
...
def _mark_notification(
    *,
    ...
    with _notification_guard(notification_path):
        ...
        target["updated_at_ms"] = now
        target["delivery"] = {
            "object_type": "AIControlOperatorNotificationDeliveryReceipt",
            "schema": "ai_control_operator_notification_delivery_receipt_v1",
...
def _notification_report(
    *,
    ...
    return {
        "object_type": "AIControlOperatorNotificationReport",
...
def _visible_delivery_guidance(notification: dict[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        "object_type": "AIControlVisibleOperatorNotificationDeliveryGuidance",
        "schema": "ai_control_visible_operator_notification_delivery_guidance_v1",
...
def _delivery_draft(notification: dict[str, Any], *, caller_id: str) -> dict[str, Any]:
    ...
    return {
        "object_type": "AIControlOperatorNotificationDeliveryDraft",
        "schema": "ai_control_operator_notification_delivery_draft_v1",
...
def _host_agent_delivery_workflow(notification: dict[str, Any]) -> dict[str, Any]:
    notification_id = notification.get("notification_id")
    return {
        "object_type": "AIControlNotificationVisibleDeliveryWorkflowPackage",
        "schema": "ai_control_notification_visible_delivery_workflow_package_v1",
        ...
        "supporting_receipt_tools": ["ai-control-tray notify mark-sent or mark-blocked"],
        "steps": [
            {
                "step_id": "preflight",
            ...
            },
            {
                ...
                "tool": f"ai-control-tray notify mark-sent --notification-id {notification_id} --evidence-path <path> --host-sent-event-count <n>",
                "blocked_tool": f"ai-control-tray notify mark-blocked --notification-id {notification_id} --reason <reason> --evidence-path <path> --host-sent-event-count <n>",
