...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Diagnose AI Control first-use readiness without sending host input.")
    parser.add_argument("--runs-dir", default="runs_first_use_doctor")
    parser.add_argument("--capture-smoke", action="store_true")
    args = parser.parse_args(argv)

    try:
        from ai_control.diagnostics.first_use_doctor import build_first_use_doctor_report
...
def _cli_failure_report(exc: Exception, *, runs_dir: str, capture_smoke: bool) -> dict[str, object]:
    ...
    return {
        ...
        "python": {
            ...
            "recommended_launcher": "py" if windows_store_alias_risk else "py_or_packaged_ai_control_first_use_doctor",
        },
        ...
        "safe_report_lines": [
            ...
            "Use py on Windows or the packaged ai-control-first-use-doctor entrypoint, then rerun the passive doctor before any real-input smoke.",
        ],
    }
