...
from ai_control.boundary.guard import BoundaryGuard, BoundaryViolation
from ai_control.capture_quality import analyze_capture_image_bytes
...
from ai_control.channels.registry import InputChannelRegistry, ObservationChannelRegistry
from ai_control.diagnostics.capture import CaptureDiagnosticsService
from ai_control.diagnostics.input import InputDiagnosticsService
from ai_control.evidence.sequence_artifacts import build_sequence_gif_artifact
from ai_control.evidence.store import EvidenceReplayService
from ai_control.failure.service import CapabilityFailureService
from ai_control.protocol.schemas import COMMAND_ORDER, PUBLIC_COMMAND_ORDER, SchemaError, schema_ref, validate_post_observe, validate_request
...
from ai_control.visual_memory.change_index import build_region_change_index
from ai_control.visual_memory.evidence_retrieval import build_visual_evidence_artifacts
from ai_control.visual_memory.frame_buffer import FrameBufferConfig, VisualFrameBuffer
from ai_control.visual_memory.post_observe import build_post_action_observation_window, should_stop_post_observe_sampling
from ai_control.visual_memory.query import query_visual_memory
...
def _segment_recorder_options_from_tray_config() -> dict[str, Any]:
    try:
        from ai_control.tray.state import default_tray_config_file, normalize_recording_policy, read_jsonc_file

        policy = normalize_recording_policy(read_jsonc_file(default_tray_config_file()))
    except Exception:
        return {}
    recording = policy.get("recording") if isinstance(policy, dict) else None
    segment = recording.get("segment") if isinstance(recording, dict) else None
    options: dict[str, Any] = {}
    if isinstance(segment, dict):
        if segment.get("storage_format"):
            options["storage_format"] = str(segment.get("storage_format"))
        if segment.get("bucket_granularity"):
            options["segment_bucket_granularity"] = str(segment.get("bucket_granularity"))
        if segment.get("image_encoding"):
            options["image_encoding"] = str(segment.get("image_encoding"))
        if segment.get("image_quality") is not None:
            options["image_quality"] = int(segment.get("image_quality") or 70)
        if segment.get("image_lossless") is not None:
            options["image_lossless"] = bool(segment.get("image_lossless"))
...
class ProtocolGateway:
    ...
    def _get_capabilities(self, payload: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "public_protocol": {
                "schema": "ai_control_screen_look_do_v1",
            ...
            "visual_memory": {
                ...
                "query_tool": {
                    ...
                    "schema": "ai_control_p0j_visual_memory_query_v1",
                    ...
                    "ai_reading_summary": {
                        "schema": "ai_control_p0j3_visual_memory_query_reading_summary_v1",
                    ...
                    "retention_prune_plan": {
                        "schema": "ai_control_p0l_visual_memory_retention_v1",
                    ...
                    "retention_class_projection": {
                        "schema": "ai_control_p0m_visual_memory_retention_class_projection_v1",
                    ...
                    "storage_attention_summary": {
                        "schema": "ai_control_p0n_visual_memory_storage_attention_summary_v1",
    ...
    def _do(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        for index, step in enumerate(seq, start=1):
            ...
            capture_windows.append(
                {
                    "kind": "burst",
                    "from": self._format_hms_ms(input_started),
                    "to": self._format_hms_ms(input_started + 3.0),
    ...
    def _ai_usage_guide(self) -> dict[str, Any]:
        return {
            ...
            "coordinate_rule": {
                ...
                "coordinate_integrity_schema": "ai_control_p1c_coordinate_integrity_v1",
            },
