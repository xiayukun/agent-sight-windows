...
- P-CONFIG-RECORDING-POLICY：托盘录制配置中心，统一 `%LOCALAPPDATA%\AgentSight\tray-config.jsonc`，只配置用户可调的 idle capture、动作前后帧、操作后 FPS / 持续时间，以及保留天数；timeline 必须启用，operation log 必须保存，不作为用户开关；
